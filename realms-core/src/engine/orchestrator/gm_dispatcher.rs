...
pub fn full_dispatch(
    ...
    // 2. 调 LLM
    let raw_response = llm.chat(&system_prompt, &req.user_message)?;
...
mod tests {
    ...
    #[test]
    fn full_dispatch_with_mock_llm() {
        let req = make_req();
        let whitelist = RoleToolWhitelist::npc_default();
        let llm =
            mock_llm("[VISIBLE]\n你好，我是林月如。\n\n[INTENT]\n这人看着面生。\n\n[EVENTS]\n[]");

        let result = full_dispatch(&req, &whitelist, &llm, &[]).expect("dispatch");
    ...
    #[test]
    fn dispatch_with_suggested_events() {
        let req = make_req();
        let whitelist = RoleToolWhitelist::npc_default();
        let llm = mock_llm(
